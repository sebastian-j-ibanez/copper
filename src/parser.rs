  ...
  match expr {
    ...
    Expr::List(list) => {
      ...
      match first_eval {
        ...
        _ => Err(
          Error::Message("first form must be a function".to_string())
        ),
      }
    },
    Expr::Func(_) => Err(
      Error::Message("unexpected form".to_string())
    ),
  }
}
...
pub fn parse<'a>(tokens: &'a [String]) -> Result<(Expr, &'a [String]), Error> {
    let (token, right_expr) = tokens.split_first()
        .ok_or(
            Error::Message("could not parse first token".to_string())
        )?;
...
pub fn parse_right_expr<'a>(tokens: &'a [String]) -> Result<(Expr, &'a [String]), Error> {
    ...
    loop {
        let (car, cdr) = tokens_copy.split_first()
            .ok_or(
